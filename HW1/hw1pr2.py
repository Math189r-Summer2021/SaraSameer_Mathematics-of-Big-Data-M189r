...
if __name__ == '__main__':
	...
	X_space_stacked = np.hstack((one_vector, X_space))  # Adding column vector for X0
	W_opt = np.linalg.solve(np.matmul(np.transpose(X_space_stacked), X_space_stacked), np.matmul(np.transpose(
	X_space_stacked), y_space_rand))

	"*** END CODE HERE ***"
	# get the new m, and new b from W_opt obtained above
	b_rand_opt, m_rand_opt = W_opt.item(0), W_opt.item(1)
	# TODO: Generate the y-coordinate of 100 points with the new parameters obtained
	# HINT:
